BackEnd 배포 자동화 구성 정리

BackEnd CI/CD 파이프라인 구성 과정은 다음과 같다.
...
1. Github Actions 를 위한 AWS 접속정보 저장
   GitHub Actions 에서는 AWS 에 접근하기 위한 secrets 를 저장하고 있어야 한다. backend 에서는 AWS 접속정보인 AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY 를 레포지토리 > Settings > 우측 배너의 Secrets and variables > Actions 에 저장한다.
   AWS_ACCESS_KEY_ID : IAM 계정 ID
   AWS_SECRET_ACCESS_KEY : IAM 계정 PW

2. S3 생성
   버킷 이름은 소문자 작성 / 이때 반드시 EC2, CodeDeploy 와 동일한 리전으로 구성
   버킷 생성 후 폴더 구조는 backend/backend 이렇게 구성한다.
   구성 예시 :
   qwerty-azit-codedeploy-artifacts
   │
   ├ backend
   │ ├ backend-deploy-a34f2.zip
   │ ├ backend-deploy-b28d1.zip
   │ └ backend-deploy-c9123.zip
   │
   └ backend
...
- CodeDeploy 구성 전 IAM 사용자 > 역할 에서 CodeDeployServiceRole (AWSCodeDeployRole) 를 등록한다.
  등록 후 CodeDeploy 애플리케이션 생성 => 배포그룹(backend/BackEnd) 생성을 진행한다.
...
이제 backend 배포 과정에 대한 전반적인 구조를 잡는다.
우선 Github Actions 로 main 브랜치 push 발생시 Runner 를 사용한 배포 결과물을 만든다. 이때 배포결과물을 만들기 위한 스크립트를 작성하여 .github > workflows 하위에 deploy-private-backend.yml 스크립트를 작성한다.
